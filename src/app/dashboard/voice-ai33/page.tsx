...
const SPEAKER_VOICE_MAP_STORAGE_KEY = "voice33_speaker_voice_map_v1";
...
export default function VoicePage() {
  const CURRENT_PROVIDER: "ai84" | "ai33" = "ai33";
  ...
  const returnToPreviousProvider = useCallback(() => {
    ...
    if (snapshot) {
      ...
      localStorage.setItem(VOICE_MIGRATION_STORAGE_KEY, JSON.stringify(payload));
      router.push(switchedFromProvider === "ai84" ? "/dashboard/voice" : "/dashboard/voice-ai33");
      return;
    }
  ...
  const runConversationQueue = useCallback(async (
    lines: ConversationLine[],
    waitingText: string
  ) => {
    const numKeys = Math.max(aliveKeys || keyCount, 1);
    const workersCount = Math.max(1, Math.min(lines.length, numKeys * Math.max(1, concurrencyLimit)));
    const queue = [...lines];
    let hardStopAll = false;

    setConversationLines(prev => prev.map((l) => {
      const shouldQueue = queue.some((q) => q.id === l.id);
      return shouldQueue ? { ...l, status: "waiting", error: waitingText } : l;
    }));

    const workers = Array.from({ length: workersCount }, (_, workerIdx) => {
      const keyIdx = workerIdx % numKeys;
      const phase = Math.floor(workerIdx / numKeys);
      return (async () => {
        if (phase > 0) {
          await new Promise(r => setTimeout(r, phase * 250));
        }
        while (!hardStopAll) {
          const line = queue.shift();
          if (!line) break;
          await new Promise(r => setTimeout(r, Math.max(0, requestDelay)));
          ...
          if (hardStop) {
            ...
            setConversationLines(prev => prev.map((item) => {
              if (item.status === "waiting" || item.status === "pending") {
                return { ...item, status: "failed", error: "Da dung som vi het key song. Bam 'Thử lại các câu lỗi'." };
              }
              return item;
            }));
            break;
  ...
  }, [aliveKeys, keyCount, concurrencyLimit, requestDelay, processLine, refreshKeyHealth]);
  ...
  const generateConversationTTS = async () => {
    ...
    await runConversationQueue(linesToProcess, "Đang trong hàng đợi...");
    setIsGenerating(false);
  };
  ...
  const retryFailedLines = async () => {
    ...
    setIsGenerating(true); setError(null);
    await runConversationQueue(failedLines, "Đang thử lại...");
    setIsGenerating(false);
  };
      ...
      <AnimatePresence>
        {showSwitchReminderModal && (
          ...
          >
            ...
            >
              <h3 className="text-base font-bold text-amber-300">Đã chuyển hệ sang AI33</h3>
