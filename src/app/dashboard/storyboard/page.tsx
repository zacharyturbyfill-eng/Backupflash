...
import { Clock, LogOut, Mic, Radio, Settings, Sparkles, Type, User, Volume2, Image as ImageIcon } from 'lucide-react';
    ...
    <div className="flex h-screen bg-[#020617] text-slate-200 font-sans overflow-hidden">
      ...
      <aside className="w-20 md:w-64 bg-[#0f172a]/80 backdrop-blur-xl border-r border-white/5 flex flex-col flex-shrink-0 z-20">
        ...
        <nav className="flex-1 py-8 px-3 space-y-2">
          ...
          <button onClick={() => router.push('/dashboard/radio-health')} className="w-full flex items-center p-4 rounded-2xl text-slate-400 hover:bg-white/5 hover:text-white transition-all group"><Radio className="w-5 h-5 flex-shrink-0 group-hover:text-indigo-400 transition-colors" /><span className="ml-3 font-medium hidden md:block">Radio Sức Khỏe</span></button>
          ...
          <button onClick={() => router.push('/dashboard/thumbnail')} className="w-full flex items-center p-4 rounded-2xl text-slate-400 hover:bg-white/5 hover:text-white transition-all group"><ImageIcon className="w-5 h-5 flex-shrink-0 group-hover:text-amber-400 transition-colors" /><span className="ml-3 font-medium hidden md:block">Thumbnail Master AI</span></button>
          {user?.role === 'admin' && (
