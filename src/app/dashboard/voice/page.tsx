...
export default function VoicePage() {
  ...
  const mergeConversationAudio = async (speakerToExport?: string) => {
    ...
    try {
      ...
      setMergedFilename(speakerToExport ? `conversation_${speakerToExport}_${Date.now()}.wav` : `conversation_full_${Date.now()}.wav`);
    } catch (err: any) {
  ...
  const exportSRT = async () => {
    ...
    try {
      ...
      const a = document.createElement("a"); a.href = url; a.download = `subtitle_${Date.now()}.txt`;
      document.body.appendChild(a); a.click(); document.body.removeChild(a);
    } catch (err: any) {
  ...
  const exportIndividualAudio = async () => {
    ...
    try {
      ...
      const a = document.createElement("a"); a.href = url; a.download = `audio_tracks_${Date.now()}.zip`;
      document.body.appendChild(a); a.click(); document.body.removeChild(a);
    } catch (err: any) {
      ...
      <aside className="w-20 md:w-64 bg-[#0f172a]/80 backdrop-blur-xl border-r border-white/5 flex flex-col flex-shrink-0 z-20">
        ...
        <nav className="flex-1 py-8 px-3 space-y-2">
          ...
          <button className="w-full flex items-center p-4 rounded-2xl bg-white/[0.03] text-white border border-white/5 shadow-lg">
            ...
            <span className="ml-3 font-semibold hidden md:block">Giọng Nói AI</span>
          </button>
