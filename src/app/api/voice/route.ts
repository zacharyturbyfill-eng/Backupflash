...
const API_BASE_URL = "https://api.ai84.pro";
...
// Lấy tất cả Minimax key + label từ vault
async function getMinimaxEntries(): Promise<KeyEntry[]> {
  const { data } = await supabaseAdmin
    .from('api_vault')
    .select('api_key, provider')
    .like('provider', 'minimax%');
  ...
  return data
    ...
    .map((d, i) => ({
      api_key: d.api_key,
      label: decodeMinimaxLabelFromProvider(d.provider) || d.provider || `Key #${i + 1}`,
    }));
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === 'get_cloned_voices') {
      ...
      results.forEach((voiceList: any[]) => {
        voiceList.forEach((voice: any) => {
          if (voice?.canonical_voice_id && !seenIds.has(voice.canonical_voice_id)) {
            seenIds.add(voice.canonical_voice_id);
    ...
    if (action === 'get_system_voices') {
      ...
      return NextResponse.json({ success: res.ok, data: data.data || [], status: res.status });
    }
      ...
      try {
        ...
        if (res.ok) {
          ...
          const { error: usageError } = await supabaseAdmin.from('usage_logs').insert({
            ...
            tool_name: `Giọng Nói AI [${entry.label}]${preview ? ` | ${preview}` : ''}`,
