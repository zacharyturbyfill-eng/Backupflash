...
function postProcess(script: string): string {
  return script
    .replace(/\([^)]{1,100}\)/g, "")      // xóa ngoặc đơn (chỉ dẫn sân khấu)
    .replace(/(?<!^|^\s*)\[[^\]]{1,100}\]/gm, "") // xóa ngoặc vuông nếu KHÔNG nằm ở đầu dòng (tránh xóa nhầm tên nhân vật)
...
async function generateOpening(
  ...
  const tokens = Math.min(65536, Math.max(3072, Math.ceil(targetChars * 0.85)));

  const systemPrompt = `
Bạn là biên kịch Radio chuyên nghiệp. Viết PHẦN MỞ ĐẦU của kịch bản Radio Sức Khỏe.
MỤC TIÊU ĐỘ DÀI: Phải đạt xấp xỉ ${targetChars} ký tự.
...
=== LUẬT KỸ THUẬT ===
- ĐỘ DÀI: Phải đạt xấp xỉ ${targetChars} ký tự. Khai thác chi tiết bối cảnh thính giả để làm dày nội dung.
- DANH XƯNG MC: Khi MC nhắc tới khách mời, BẮT BUỘC dùng danh xưng đầy đủ: "${derived.guestFullLabel}". TUYỆT ĐỐI KHÔNG gọi tên trống không.
- Mỗi lời thoại BẮT BUỘC bắt đầu bằng: Tên nhân vật: [nội dung] (KHÔNG dùng ngoặc vuông cho tên).
- TUYỆT ĐỐI CẤM: ngoặc đơn, ngoặc vuông, chỉ dẫn sân khấu, mô tả nội tâm, hiệu ứng âm thanh (tiếng rè, chuyển máy...).
...
async function generateCounseling(
  ...
  const tokens = Math.min(65536, Math.max(6144, Math.ceil(targetChars * 0.85)));
...
- Trọng tâm: ${phaseIndex === 0 
    ? "Phân tích sâu về giá trị nhân phẩm và sự tử tế trong lao động chân chính (theo synopsis). Khai thác bối cảnh cuộc sống của thính giả." 
    : "Cảnh tỉnh về sự tha hóa đạo đức do đồng tiền và lòng tham. Đưa ra các ví dụ và phân tích tâm lý sâu sắc."}
- Yêu cầu nội dung: Các lời thoại của ${derived.guestShortLabel} phải mang tính triết lý, nhân văn, phân tích đa chiều để đạt độ dài cần thiết.
- "${derived.callerDisplayName}" phản hồi chi tiết về những trải nghiệm, khó khăn và suy nghĩ của mình.
...
=== LUẬT KỸ THUẬT ===
- ĐỘ DÀI: Phải đạt mục tiêu ${targetChars} ký tự. Hãy viết chi tiết, sâu sắc, không tóm tắt ý.
- Mỗi lời thoại BẮT BUỘC bắt đầu bằng: Tên nhân vật: [nội dung] (KHÔNG dùng ngoặc vuông cho tên).
...
=== ĐỊNH DẠNG LỜI THOẠI BẮT BUỘC ===
- ĐỘ DÀI: Phải đạt mục tiêu ${targetChars} ký tự.
- DANH XƯNG MC: Khi MC nhắc tới khách mời, BẮT BUỘC dùng danh xưng đầy đủ: "${derived.guestFullLabel}".
- Mỗi lượt thoại viết trên MỘT DÒNG DUY NHẤT: Tên nhân vật: [nội dung]

TUYỆT ĐỐI CẤM:
- Lặp lại các lời khuyên đã nói ở phần tư vấn trước đó.
- Mô tả nội tâm hoặc hành động nhân vật, hiệu ứng âm thanh.
- Chỉ dẫn sân khấu trong ngoặc đơn () hoặc ngoặc vuông [].
...
export async function POST(req: NextRequest) {
  try {
    ...
    const guestRoleType  = cfg.doctorRole === "DOCTOR" ? "Bác sĩ" : "Chuyên gia";
