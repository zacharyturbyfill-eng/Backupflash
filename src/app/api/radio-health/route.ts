...
function postProcess(script: string): string {
  return script
    .replace(/\([^)]{1,40}\)/g, "")       // xóa chỉ dẫn sân khấu trong ngoặc đơn
    .replace(/\[[^\]]{1,40}\]/g, "")      // xóa chỉ dẫn sân khấu/âm thanh trong ngoặc vuông
...
- Mỗi lượt thoại bắt đầu bằng: [Tên nhân vật]: [nội dung]
